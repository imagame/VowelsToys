package com.imagame.game 
{
	 ...
	 * @author imagame
	 */
	public class PanelMenuFase1 extends ImaPanel 
	{
		...
		public function PanelMenuFase1(id:uint) 
		{
			...
			_dlg = ImaCachedBitmap.instance.createBitmap(Assets.GfxDlgMenuPhase1); 
			...
			_tileSheet = new ImaBitmapSheet(Assets.GfxButtonsPanelMenuFase1, Assets.BUTTON_LEVELVO_WIDTH, Assets.BUTTON_LEVELVO_HEIGHT); //frames sorted: from left to right, up to down, vowel an its levels. 
			...
			_subtileSheet = new ImaSubBitmapSheet(Assets.GfxButtonsPanelMenuFase1, Assets.BUTTON_LEVELNO_WIDTH, Assets.BUTTON_LEVELNO_HEIGHT, 
			...
			//init level buttons: init() function will be automatically called, and the level buttons will be enabled (depending on max level reached)
		...
		override public function init():void { 
			...
			for (var i:uint = 0; i < 5; i++) { //5 vowelds 
				...
				var maxLvl:int = (Registry.gpMgr as PropManager).getLevelProgress(i); //Max Level arrived: 2-> lvl-1 completed,.., 6-> lvl-5 completed 
		...
		public function onLevelClick(event:MouseEvent):void { 
			...
			//idVowel = idVal / 6; //0: A, 1: E, 2:I, 3:O, 4:U 
			//idLvl = idVal % 6; //0: bodypart select, 1..5: puzzle difficulty level 
			if (idval % 6 == 0 )
		...
		private function onTweenButtonOn(pidx: int):void { 
			...
			var maxLvl:int = (Registry.gpMgr as PropManager).getLevelProgress(pidx);
